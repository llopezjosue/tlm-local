...
- **Consistency/indicator signals change score calibration, not just latency**: both are inactive at `quality_preset=medium` (0 completions for a QA workflow), so the default preset scores on `self_reflection_score` plus `perplexity_score` only, in a roughly 70/30 split once the missing signals are renormalized away. Moving to `high` is not the same score made stricter: a third of the weight shifts onto consistency, which measures resampling stability rather than accuracy, and a good answer measured here dropped from ~0.9 to ~0.72 without any error being detected. Adopting it means re-tuning thresholds, not just accepting more latency. See [`../docs/SCORING.md`](../docs/SCORING.md).
...
- **A score does not say which signals produced it**: `tlm` computes the five per-signal sub-scores and then drops them, so `ScoreResult.raw` carries the final number but no decomposition, and a score built from one surviving judge is indistinguishable from one built from six. They can only be read out of `tlm`'s own INFO logs, which is scraping, not an API; the recipe is in [`../docs/SCORING.md`](../docs/SCORING.md).
